 ...
 * snippet, and the downloadable summarize.js file. It is kept in lockstep
 * with extension/widget.js — update both together.
 *
 ...
 *
 * Hardened: Shadow DOM isolation, no lookbehind regexes, void-wrapped so it
 * can never blank a host page, wrapped in try/catch.
 */
export const WIDGET_SOURCE = `(function () {
  ...
  try {
    ...
    function render(_variance) {
      ...
      // so call sites reflect intent — see the comment at its usage.
      ...
      // Single source of truth for stopping narration — used by Close,
      // Shuffle, the points dropdown, and the Listen button itself so the
      // label can never get stuck showing "Stop" while nothing plays.
 ...
 * Wrapped in void(...) so it always evaluates to undefined and can never
 * blank the host page.
 */
